interface Token<T extends keyof HTMLElementTagNameMap, E = HTMLElementTagNameMap[T]> {
  type: T;
  length: number;
  text: string;
  tagName: string;
...
export interface PostScribeProps {
  ...
  beforeEnqueue?: (tagName: string, content: string, options: Partial<Omit<PostScribeProps, 'html'>>) => void;
  /** A callback called before writing buffered document.write calls */
  beforeWrite?: () => void;
  /** A callback called before writing a token */
  beforeWriteToken?: <T extends keyof HTMLElementTagNameMap>(token: Token<T>) => Token<T>;
